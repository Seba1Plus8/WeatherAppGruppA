...
//-----------------denna funktion körs varje gång man skriver in en bokstav på sökrutan
    async function updateValue() {
        ...
        if (input.value.length > 0) {
            ul.innerHTML = 
            `
            <li class="search-item"><span class="star-icon"></span><a href="#" id="city-result">${data.results[0].name}, ${data.results[0].country}</a>
            </li>
            ` 
        }
        
        favorit = document.querySelector(".star-icon");
        const a = document.querySelector("#city-result");
